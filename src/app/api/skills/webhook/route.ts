...
/**
 * POST /api/skills/webhook
 * Webhook called by MCP server when a user adds a tabular item
 * Determines sync direction and triggers the sync
 */
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`🔔 Webhook triggered: user=${workos_user_id}, skill=${skill_name}`)
    ...
    const syncResponse = await fetch(`${cleanBaseUrl}/api/skills/sync`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },

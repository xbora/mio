...
async function getMcpTools(bearerToken: string) {
...
export async function POST(req: Request) {
  ...
  // Use MCP_MIO_BEARER_TOKEN from secrets for dev testing
  const bearerToken = process.env.MCP_MIO_BEARER_TOKEN;
  
  if (!bearerToken) {
    mcpError = 'MCP_MIO_BEARER_TOKEN not configured in secrets';
    console.error(mcpError);
  } else {
    try {
      const mcpResult = await getMcpTools(bearerToken);
